...
def _build_prompts(
    ...
    system_prompt = prompt_overrides.get(
        ...
        (
            ...
            "Always generate fresh, original Python code. Never reproduce long passages of "
            "copyrighted or training data code verbatim; instead, synthesize new implementations "
            "based only on the user's UML diagram and instructions. "
            "Generate code that adheres to the user's specification, focuses on clarity, "
            "and complies with the wait_input() contract for interactive flows. "
            "If the UML diagram contains mistakes, missing pieces, or conflicting models, "
            "do not auto-complete or fix them—produce code exactly as depicted in the diagram. "
            "All user-visible messages printed by the program (via print()) must be written in natural Japanese. "
            "If you include code comments, prefer writing them in Japanese."
        ),
    )
    instructions = prompt_overrides.get("additional_instructions", "")
    user_prompt = (
        "Analyze the UML diagram provided as Base64 data and generate runnable Python code.\n"
        f"- Session ID: {session_id}\n"
        f"- Diagram type: {diagram_type}\n"
        "- The code must:\n"
        "  1. Run in a single file with a main() entry point guarded by if __name__ == '__main__'.\n"
        "  2. Rely only on the Python standard library.\n"
        "  3. Use the helper function wait_input() as a host-provided function.\n"
        "     IMPORTANT CONSTRAINTS for wait_input():\n"
        "     - Do NOT define wait_input() in the generated code.\n"
        "     - Do NOT include any comments explaining what wait_input() does.\n"
        "     - wait_input() returns ONLY 'A' or 'B'. No other values are possible.\n"
        "     - The host application has exactly TWO buttons: 'A' and 'B'.\n"
        "     - If the UML diagram implies more than two distinct actions, you MUST map them\n"
        "       creatively to A and B. For example:\n"
        "       * A = toggle power (ON<->OFF)\n"
        "       * B = toggle another feature (e.g., child lock ON<->OFF)\n"
        "     - NEVER use 'C', 'D', 'Q', or any other input values.\n"
        "     - Call wait_input() once per input cycle, store the result, and branch on it.\n"
        "     - Do not call wait_input() repeatedly inside a tight loop.\n"
        "  4. Avoid infinite busy loops; include sleep calls where appropriate.\n"
        "  5. Contain descriptive print statements that explain state transitions.\n"
        "  6. Do NOT include comments that explain host-provided functions (like wait_input()).\n"
        "     The code should be clean and self-explanatory without meta-comments.\n"
        "- Do not include Markdown explanations. Return ONLY the Python source code, preferably inside "
        "a single triple-backtick block tagged with python.\n"
    )
    if instructions:
